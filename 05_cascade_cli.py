...
def collect_links(
    ...
    for i in range(min(total, max_links)):
        a = anchors.nth(i)
        # ... após o loop de scroll
    try:
        items = page.evaluate("""
            () => {
                const q = (root) => Array.from((root || document).querySelectorAll('a[href*="/web/dou/"]'));
                const root = document.querySelector('#hierarchy_content') || document;
                return q(root).slice(0, %d).map(a => ({
...
def gen_bulletin(result: Dict[str, Any], kind: str, out_path: str) -> None:
    ...
    if kind == "docx":
        ...
        def _add_hyperlink(paragraph, url: str, text: str, color="0000FF", underline=True):
            """Insere um hyperlink clicável em um parágrafo do python-docx."""
        ...
        for (org, tipo), arr in groups.items():
            ...
            for it in arr:
                ...
                if durl:
                    _add_hyperlink(p, durl, titulo)
                else:
                    p.add_run(titulo)
                if pdf:
                    p.add_run(" [")
                    _add_hyperlink(p, pdf, "PDF")
                    p.add_run("]")
                if suffix:
                    p.add_run(suffix)
...
def flow_batch(p, args):
    """
    Executa um plano (config JSON) em modo batch.

    Com --reuse-page: reaproveita uma única aba por (data, secao) para todos os jobs do grupo,
    evitando novo goto() e abertura/fechamento de abas a cada job.
    """
    from collections import defaultdict
    import hashlib
    ...
    # >>> IMPORTANTÍSSIMO: 'jobs' é criado AQUI e só é usado abaixo <<<
    ...
    try:
        if getattr(args, "reuse_page", False):
            # ===== RAMO COM REUSO DE ABA =====
            groups = defaultdict(list)
            # preserva um índice global (para nome do arquivo)
                ...
                try:
                    ...
                    for global_idx, job in gjobs:
                        try:
                            ...
                            state_file = job.get("state_file") or cfg.get("state_file")
                            ...
                            # --- Dedup simples via state_file (hash do link/detail_url) ---
                            filtered = enriched
                            if state_file:
                                seen = set()
                                state_path = Path(state_file)
            ...
            for j_idx, job in enumerate(jobs, 1):
                ...
                key1_type = job.get("key1_type"); key1 = job.get("key1")
                key2_type = job.get("key2_type"); key2 = job.get("key2")
                key3_type = job.get("key3_type"); key3 = job.get("key3")
                label1 = job.get("label1"); label2 = job.get("label2"); label3 = job.get("label3")
                query = job.get("query")
                ...
                bulletin_out = None
                if bulletin and bulletin_out_pat:
                    bulletin_out = out_dir / render_out_filename(bulletin_out_pat, {**job, "_job_index": j_idx})
                ...
                try:
                    ...
                    flow_run(
                        context, data, secao,
                        key1, key1_type, key2, key2_type, key3, key3_type,
                        query, max_links, str(out_path), debug_dump,
                        scrape_detail, detail_timeout, fallback_date,
                        ...
                        bulletin=bulletin, bulletin_out=str(bulletin_out) if bulletin_out else None
                    )

                    try:
                        saved = json.loads(Path(out_path).read_text(encoding="utf-8"))
                        report["items_total"] += int(saved.get("total", 0))
                    except Exception:
                        pass
...
def main():
    ...
    ap.add_argument("--no-level3", action="store_true", help="(obsoleto no plan: N3 é ignorado)")  # opcional
    ap.add_argument("--plan-verbose", action="store_true", help="Mostra diagnóstico detalhado durante o plan")
    ap.add_argument("--max-combos", type=int, help="(plan) Corta o produto L1×L2 para no máximo N combos")

    # PLAN from pairs
    ap.add_argument("--pairs-file", help="JSON de pares N1->N2 gerado pelo 00_map_pairs.py")
    ap.add_argument("--max-combos", type=int, help="(plan) Corta o total de combos para no máximo N")
