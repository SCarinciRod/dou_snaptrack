...
def wait_dropdown_ready(frame, timeout_ms: int = 2000, min_options: int = 1, poll_ms: int = 50) -> bool:
    ...
    while (time.time() - start_time) * 1000 < timeout_ms:
        try:
            ...
            visible_dd = frame.locator('.selectize-dropdown[style*="display: block"], .selectize-dropdown:not([style*="display: none"])').first
...
def open_selectize_fast(page, frame, selectize_control: dict, timeout_ms: int = 2000) -> bool:
    ...
    try:
        ...
        if not ready:
            logger.warning(f"Dropdown não ficou pronto em {timeout_ms}ms")
...
def map_eagendas_pairs_fast(
    ...
    try:
        ...
        if not open_selectize_fast(page, frame, dd_orgao, timeout_ms=4000):
        ...
        for idx_org, orgao in enumerate(orgaos, 1):
            ...
            dd_orgao = cache.get_control("Órgão ou entidade")
            if not open_selectize_fast(page, frame, dd_orgao, timeout_ms=3000):
                logger.error(f"Erro ao abrir dropdown de órgãos")
                continue
            
            # Selecionar órgão
            if not select_option_fast(page, frame, orgao, wait_ajax=True, timeout_ms=5000):
            ...
            if not open_selectize_fast(page, frame, dd_cargo, timeout_ms=1500):
                result["hierarchy"].append(orgao_entry)
                continue
            ...
            for idx_cargo, cargo in enumerate(cargos, 1):
                # Abrir cargo
                dd_cargo = cache.get_control("Cargo")
                if not open_selectize_fast(page, frame, dd_cargo, timeout_ms=1000):
                    continue
                ...
                if not open_selectize_fast(page, frame, dd_agente, timeout_ms=1500):
                    orgao_entry["cargos"].append(cargo_entry)
                    continue
