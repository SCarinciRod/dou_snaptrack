...
try:
    ...
    def open_dropdown_robust(page_or_frame, handle):
    ...
    def collect_open_list_options(page_or_frame):
...
def map_eagendas_pairs(
    ...
    try:
        ...
        orgaos = get_selectize_options(frame)
        ...
        for idx_org, orgao in enumerate(orgaos, 1):
            ...
            if not select_selectize_option(page, orgao, wait_after_ms=2000):
                logger.error(f"Erro ao selecionar órgão '{orgao.get('text')}'")
                close_selectize_dropdown(page)
                continue
            ...
            if not open_selectize_dropdown(page, dd_cargo, wait_ms=1000):
                logger.warning(f"  Não foi possível abrir dropdown de Cargos")
                result["hierarchy"].append(orgao_entry)
                continue
            
            cargos = get_selectize_options(frame)
            ...
            for idx_cargo, cargo in enumerate(cargos, 1):
                ...
                if not open_selectize_dropdown(page, dd_cargo, wait_ms=800):
                    logger.error(f"    Erro ao abrir dropdown de cargos")
                    continue
                ...
                if not open_selectize_dropdown(page, dd_agente, wait_ms=1000):
                    logger.warning(f"      Não foi possível abrir dropdown de Agentes")
                    orgao_entry["cargos"].append(cargo_entry)
                    continue
                
                # Excluir a opção genérica "Todos os ocupantes"
                agentes = get_selectize_options(frame, exclude_patterns=["todos os ocupantes"])
                close_selectize_dropdown(page, wait_after_ms=500)
        ...
        if verbose:
            ...
            logger.info(f"  Total de órgãos processados: {len(result['hierarchy'])}")
