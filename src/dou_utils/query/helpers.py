"""Helper functions for query_utils - link collection and scrolling."""

import contextlib
import re


def find_search_box(frame, query: str):
...
def _fill_search_box(sb, query: str):
    """Fill search box with fallback to JS."""
    try:
        sb.fill(query)
    except Exception:
        sb.evaluate(
...
def _submit_search(frame, sb):
    """Submit search via Enter or button click."""
    try:
        sb.press("Enter")
    except Exception:
        try:
...
def _wait_for_results(frame):
    """Wait for content to appear after search."""
    with contextlib.suppress(Exception):
...
def find_best_frame_and_locator(page, frame):
...
def try_load_more_button(active_frame, page, max_attempts=5):
...
def scroll_to_load_links(active_frame, page, anchors, max_links, max_scrolls, scroll_pause_ms, stable_rounds):
    """Scroll frame and page to load more links."""
    last = -1
    stable = 0
    last_scroll_h = None
...
def extract_links_vectorized(active_frame, max_links):
    """Extract links using vectorized JavaScript evaluation."""
    try:
...
def extract_links_fallback(anchors, max_links):
    """Fallback method to extract links one by one."""
    items = []
    try:
        total = anchors.count()
    except Exception:
